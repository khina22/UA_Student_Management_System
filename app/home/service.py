...
def track_std():
    ...
    index_number = request.form['std_index']
    checkIndexorCid='SELECT spp.student_cid,index_number FROM public.tbl_students_personal_info AS spp \
    ...
    indexorCid=engine.execute(checkIndexorCid,student_cid,index_number).fetchall()
    if indexorCid ==[]:
     ...
     str_query = "SELECT *, spp.id as std_id FROM public.tbl_students_personal_info AS spp " \
            ...
            "ON ac.std_personal_info_id = spp.id " \
            "WHERE spp.student_cid = %s AND ac.index_number = %s"
     std_list = engine.execute(str_query, student_cid, index_number).fetchall()
...
def getClassIdGeneral():
    query = text("SELECT class_id,class_name FROM public.class LIMIT 3")
    result = connection.execute(query)
    # Convert the result into a list of dictionaries
    dropdown_values = [
        {'id': row.class_id, 'name': row.class_name}
...
def getstudentDetail():
     ...
     public.tbl_academic_detail b ON std.id=b.std_personal_info_id \
     where (std.student_cid =%s OR b.index_number=%s) AND std.status='approved'"
...
def checkIndexExists(index):
    check_exist_index= text('SELECT COUNT(*) FROM public.tbl_students_personal_info as sp INNER JOIN public.tbl_academic_detail as ac ON ac.std_personal_info_id = sp.id  WHERE index_number = :index')
    results = connection.execute(check_exist_index, {"index": index}).fetchone()[0]
    output = int(results)
    if output > 0:
        return True
    else:
...
def pay_std_fee(studentCid):
    ...
    if paymentModes=='Seat Confirm' and countedSeatConfirm>0:
        ...
        if paymentModes=='Full':
            if countedInstall>=1 and countedInstall is not 4:
    ...
    #     return {"error": "You have already paied all the school fee in full payment"}
    connection.execute("INSERT INTO public.tbl_acc_detail (id, f_name,l_name, jrn_number, bank_type, acc_holder, std_class, std_cid,amount,paymentModes,screen_shot) "
